...
The IYREE Python SDK provides a convenient interface to the IYREE platform, including DWH (StarRocks), Cube.js analytics, S3 object storage, a Key-Value store, and Reports.
...
<ResponseField name="gateway_host" type="str">
  Gateway base URL (e.g. `https://api.iyree.io`). Falls back to the `IYREE_GATEWAY_HOST` environment variable. If the value does not start with `http://` or `https://`, `https://` is prepended automatically.
</ResponseField>
...
After initialization, access the sub-clients as properties:
...
| `client.reports` | [Reports](/reports) — Create reports from IYREE functions and jobs |
