"""Sync Reports client integration tests with respx mocking."""
...
@pytest.fixture
def reports(transport: HttpTransport, config: IyreeConfig) -> ReportsClient:
    return ReportsClient(transport, config)
...
class TestReportsCreateSync:
    @respx.mock
    def test_create_happy_path(self, reports: ReportsClient, gateway_host: str):
    ...
    def test_create_serializes_datetimes(self, reports: ReportsClient, gateway_host: str):
        ...
        assert sent["function_path"] == "/scripts/run.py"
        assert sent["job_id"] == "job_1"
        assert sent["html_content"] == "<html></html>"

    @respx.mock
    def test_create_omits_optional_fields(self, reports: ReportsClient, gateway_host: str):
        route = respx.post(f"{gateway_host}/api/v1/reports").mock(
            return_value=httpx.Response(201, json=REPORT_RESPONSE)
        )
        reports.create(
    ...
    @respx.mock
    def test_create_includes_all_optional_fields(self, reports: ReportsClient, gateway_host: str):
        route = respx.post(f"{gateway_host}/api/v1/reports").mock(
            return_value=httpx.Response(201, json=REPORT_RESPONSE)
        )
        main_period = ReportPeriod(
        ...
        compare_period = ReportPeriod(
            date_from=datetime(2026, 5, 5, tzinfo=timezone.utc),
            date_to=datetime(2026, 5, 12, tzinfo=timezone.utc),
        )
        reports.create(
        ...
        sent = json.loads(route.calls[0].request.content)
        assert sent["title"] == "Weekly summary"
        assert sent["description"] == "Lorem ipsum"
        assert sent["locations"] == [1, 2, 3]
        assert sent["compare_period"]["date_from"] == "2026-05-05T00:00:00+00:00"
        assert sent["compare_period"]["date_to"] == "2026-05-12T00:00:00+00:00"
        assert sent["summary"] == "All good"
        assert sent["name"] == "weekly-summary"
        assert sent["workspace_id"] == "ws_1"

    @respx.mock
    def test_create_accepts_dict_period(self, reports: ReportsClient, gateway_host: str):
        route = respx.post(f"{gateway_host}/api/v1/reports").mock(
            return_value=httpx.Response(201, json=REPORT_RESPONSE)
        )
        reports.create(
    ...
    @respx.mock
    def test_create_parses_envelope_response(self, reports: ReportsClient, gateway_host: str):
        respx.post(f"{gateway_host}/api/v1/reports").mock(
    ...
    @respx.mock
    def test_create_handles_null_nested_fields(self, reports: ReportsClient, gateway_host: str):
        minimal_response = {
            "id": "rep_minimal",
            "creation_type": "PREDEFINED",
            "main_period": {
